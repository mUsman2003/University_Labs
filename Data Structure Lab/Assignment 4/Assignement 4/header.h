...
public:
	...
	taskManagementSystem() :root(NULL),task(NULL)
	{
	}
	...
	void PriorityUpdator(Task* temp, int id, int p)
	{
		...
		if (id == temp->TaskID)
		{
			...
			Task* ptr=new Task;
			ptr->TaskID = temp->TaskID;
			ptr->AssigneeID = temp->AssigneeID;
			ptr->Priority = p;
			ptr->Description = temp->Description;

			deleteTask(id);

			addTask(ptr->TaskID, ptr->Description, p, ptr->AssigneeID);
			delete ptr;
			ptr = NULL;
			return;
		}
	...
	void LeastPriority(Task* temp, int p,stringstream& result)
	{
		if (temp == NULL)
			return;
		LeastPriority(temp->left, p,result);
		if(p==temp->Priority)
		...
		LeastPriority(temp->right, p,result);
